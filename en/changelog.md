...
This page is generated from the real Git history of the main repository (1668 commits, 2022-12-06 through 2026-09-18). It includes **feature implementation and bug-fix commits only**.

**Exclusion rules**: `chore` commits (frontend build artifacts), merge commits, README/documentation commits, automatic dependency bumps, `Initial plan`/synthetic commits, CI workflow commits, and vague/trivial subjects (e.g. "detail optimization"). Identical subjects on the same day are kept once. Subjects are the original commit messages with Markdown-reserved characters escaped; no invented rewrites of ambiguous semantics.

> The repository has no tags and `package.json` `version` is always `1.0.0`. This page therefore does not use fictional version numbers — entries are listed by year and month of the real commit dates. Commit subjects remain in their original language (mostly Chinese).

## September 2026 in development (annotation)

> Annotated root causes and scope of this round of fixes/features, corresponding to the commits listed under the 2026-09 section below.

### Bug fixes

| Topic | Root cause & handling |
| --- | --- |
| Tool call ID normalization | Old data / upstream relays send array indices as tool-call IDs (numeric), causing 400/500 on OpenAI-compatible endpoints. Added `generateDeterministicToolCallId`/`normalizeAnyToolCallId` in `tooling.js` and applied across converters, `OpenAIClient`, `AbstractClient` and `ToolApprovalService`. |
| Gemini `functionResponse.name` 400 | Missing `tcr.name` was forwarded as-is and nameless parts were silently dropped. Multi-level fallback via `resolveToolResultName` guarantees a non-empty name; functionCall IDs (incl. streaming) now use deterministic IDs. |
| Gemini `image/gif` 500 | Gemini endpoint whitelist lacks image/gif. New `getImageSendPolicy`/`transcodeImageForClient` transcoding chain: out-of-whitelist images (gif/bmp/svg...) are sharp-transcoded to **image/webp** (animations preserved), fallback to static JPEG, then to a text placeholder; both streaming and non-streaming preprocessing pass `'gemini'`; converter adds `GEMINI_IMAGE_MIME_WHITELIST` guard. |
| At-mention trigger broken | icqq/TRSS vs QQBot at-segment shapes and botId matching were incomplete. Added `e.atBot` + segment `qq`/`data.qq`/`data.user_id`, `e.atme`, `data.all` (@everyone), botId fallback chain; fixed the replyBot truth-table swallowing messages. |
| Disabled channels still visible | Backend aggregation did not filter `enabled === false`. Filtered in backend and all frontend aggregation points (`useConfig`, group editor, group-admin, users page, imagegen). |
| Memory "organize" frontend no-op | Summary threshold always skipped; `hard === 'true'` never true. Lowered threshold (bucket >2, category total ≥5), added `POST /api/memory/user/:userId/cleanup`, fixed `hard` param parsing. |
| Dropdown overflow (options unreachable) | Missing heights/`collisionPadding` on `SelectContent`/`Viewport`. Fixed all overflow points (memory page 260px, ModelSelector, ModelMappingEditor...). |
| chatdebug raw JSON output | Double-wrapping and field errors. Now goes through `StandardBotApi.sendForward({nodes})`; `di.channel.channelId → di.channel.id`; default text for missing timing. |
| Error rethrow rework (retry first, no flooding, log-only by default) | Adapter inline reply lived inside the retry loop (up to 4 user messages per error) and falsely claimed history had been cleaned. Removed it; errors exhaust the ChatService retry chain before rethrow; `chat.js` logs only when `errorNotify` is off; auto-clean still performs real deletion then notifies if `notifyUser`. |
| `server` errors never retried | ChatService classification lacked `server`. Added `server` (5xx) to backoff/key/channel-switch whitelist; `client` stays on plain retry budget. |
| Error notification semantics | `notify` returned "sent?", so failed targets let errors fall back to the triggering user. Now returns "handled": enabled = always silent; disabled = friendly message only via logs. |

### Implementations

| Topic | Notes |
| --- | --- |
| LlmDelegate unified bypass calls | Side-channel LLM calls (memory, KG, context summaries) now follow channel `advanced.streaming`, classified error reporting, channel switching with exponential backoff; wired into MemoryManager/KnowledgeGraphExtractor/llmHelper/ContextManager/ConversationTracker/GalgameService. |
| Knowledge graph toolset | 12 `kg_*` tools in `src/mcp/tools/knowledgeGraph.js`, category `knowledgeGraph` enabled by default; `preserveTargetId` normalizes per-protocol user/group ids (QQBot adaptation). |
| Proactive model memory | `ChatAgent._addToolPrompt` injects `KNOWLEDGE_GRAPH_TOOL_GUIDE`/memory-maintenance guides; `_triggerMemoryExtraction` carries the last 10 history messages. |
| Structured memory summaries | `[分类] 内容` hard format with category whitelist; dual-channel parser prevents thinking-process text from polluting memories. |
| Auto-polling memory completion | Poll cursor persisted in `kv_store` (survives restart); conversations are settled before history is cleared (both Commands and Management paths). |
| Tool call limits redefined | Only 4 consecutive fully-identical calls (tool name + args, byte-identical) suspend tools via `toolChoice:none`; different args are never blocked or warned; all round/total-count caps removed. |
| Intermediate tool text | Text-less tool rounds emit a lightweight "calling..." notice through `onMessageWithToolCall`; intermediate texts are no longer dropped. |
| Canvas rendering | Puppeteer replaced by `canvasRenderer.js` (`@napi-rs/canvas` + `mathjax-full`); 7 render methods; katex direct dependency removed. |
| Streaming on by default | `ChannelManager.normalizeAdvanced` treats missing streaming config as enabled; 3 frontend defaults flipped to true. |
| Debug completion & nested forwards | `sendDebugInfo` extended to 9 submodules; `StandardMessage.nestedForward` with chunking. |
| Security hardening | Safe-path/SSRF prechecks for builtin tool fs/http; preset template `${expression}` replaced with dotted-path safe access. |
| Media probe-byte plan | `probeImageHeader` reads first 4KB via `Range` + magic-number detection instead of HEAD; QQ image-host domains unified. |
| Context & persona | Group shared context; tokenized `acquireLock` deferred queue; unified persona priority; isolated persona context via `isolateContext`. |
| Memory system activation | `structured_memories` gains `last_access_at`/`access_count`; query-relevance sorting; `decayConfidence` by access time. |
| Auto-clean on error | `autoCleanOnError`: real conversation deletion (old + new IDs) after retry exhaustion, with 「历史对话已自动清理」 notice when `notifyUser`. |
