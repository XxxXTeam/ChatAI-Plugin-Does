...
| 类别 | 图标 | 说明 | 示例工具 |
|:-----|:----:|:-----|:---------|
| **basic** | 🔧 | 基础功能 | `get_time`, `get_weather`, `calculate` |
| **user** | 👤 | 用户信息 | `get_user_info`, `get_user_avatar` |
| **web** | 🌐 | 网络功能 | `web_search`, `fetch_url` |
| **group** | 👥 | 群组操作 | `get_group_info`, `get_member_list` |
| **memory** | 🧠 | 记忆管理 | `save_memory`, `search_memory` |
| **media** | 🎨 | 媒体处理 | `generate_image`, `text_to_speech` |
| **message** | 💬 | 消息操作 | `send_message`, `recall_message` |
| **schedule** | ⏰ | 定时任务 | `set_reminder`, `create_schedule` |
